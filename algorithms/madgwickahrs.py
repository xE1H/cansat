...
import math
import algorithms.umatrix as umatrix
import algorithms.ulinalg as ulinalg
...
class MadgwickAHRS:
    ...
    def update(self, gyroscope, accelerometer, magnetometer, sample_period=None):
        """
        Perform one update step with data from a AHRS sensor array
        :param gyroscope: A three-element array containing the gyroscope data in radians per second.
        :param accelerometer: A three-element array containing the accelerometer data. Can be any unit since a normalized value is used.
        :param magnetometer: A three-element array containing the magnetometer data. Can be any unit since a normalized value is used.
        :param sample_period: The sample period (optional)
        :return:
        """
        if sample_period is not None:
            self.samplePeriod = sample_period / 1000
        ...
        h = q * (Quaternion(0, magnetometer[0, 0],
                            magnetometer[1, 0],
                            magnetometer[2, 0]) * q.conj())
        ...
        f = umatrix.matrix(
            ...
            [2 * (q[1] * q[3] - q[0] * q[2]) - accelerometer[0, 0],
             ...
             2 * b[1] * (q[1] * q[2] - q[0] * q[3]) + 2 * b[3] * (q[0] * q[1] + q[2] * q[3]) - magnetometer[1, 0],
             2 * b[1] * (q[0] * q[2] + q[1] * q[3]) + 2 * b[3] * (0.5 - q[1] ** 2 - q[2] ** 2) - magnetometer[2, 0]
        ...
        qdot = ((q * Quaternion(0, gyroscope[0, 0], gyroscope[0, 1], gyroscope[0, 2])) * 0.5 -
                Quaternion(step.transpose() * self.beta))

        # Integrate to yield quaternion
        q = q + qdot * self.samplePeriod
        self.quaternion = q * (1 / ulinalg.norm(q._q))  # normalise quaternion

    def update_imu(self, gyroscope, accelerometer, t=None):
        """
        ...
        # Compute rate of change of quaternion
        qdot = (q * Quaternion(0, gyroscope[0, 0], gyroscope[0, 1], gyroscope[0, 2])) * 0.5 - Quaternion(
            step.T * self.beta)
